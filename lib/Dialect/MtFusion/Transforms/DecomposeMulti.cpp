...
#define DEBUG_TYPE "decompose-multi"
...
struct DecomposeMultiPattern : public OpRewritePattern<LinalgType> {
  ...
  LogicalResult matchAndRewrite(LinalgType linalgOp,
    ...
    unsigned inputNum = linalgOp.getNumDpsInputs();
    unsigned outputNum = linalgOp.getNumDpsInits();
  ...
  void outline(PatternRewriter &rewriter, linalg::LinalgOp linalgOp,
    ...
    for (int i = newBlock.getNumArguments() - 1; i >= 0; --i) {
      if (!indices.count(i)) {
...
std::unique_ptr<Pass> mlir::mtfusion::createDecomposeMulti() {
