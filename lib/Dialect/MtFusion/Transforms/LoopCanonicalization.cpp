...
static bool isShapePreserving(scf::ForOp forOp, int64_t arg) {
...
struct DimOfIterArgFolder : public OpRewritePattern<OpTy> {
  ...
  LogicalResult matchAndRewrite(OpTy dimOp,
    ...
    if (!blockArg)
      return failure();
    auto forOp = dyn_cast<scf::ForOp>(blockArg.getParentBlock()->getParentOp());
    if (!forOp)
      return failure();
...
std::unique_ptr<Pass> mtfusion::createSCFForLoopCanonicalization() {
